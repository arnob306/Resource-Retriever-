...
def run_index(
    ...
    for record in store.list_all(source_type="local"):
        ...
        _apply_action(record, discovered, store, vector_store, embedder, app_config, counts)
...
def _resolve_discovered_state(record: FileRecord, force_rehash: bool) -> Optional[DiscoveredState]:
    ...
    unchanged = current_mtime == record.modified_time and current_size == record.file_size and record.content_hash
...
def _apply_action(
    ...
    if action in (Action.NEW, Action.REPROCESS):
        _embed_file(record, discovered, resolved_hash, store, vector_store, embedder, app_config, counts)
...
def _embed_file(
    ...
    except (OSError, ExtractionError, ChunkingError):
    ...
    counts["new" if is_new else "reprocessed"] += 1
...
def _purge_stale(store: MetadataStore, vector_store: VectorStore, processed_ids: set[str]) -> int:
    stale = store.list_stale(processed_ids)
    for record in stale:
        vector_store.delete_by_file_id(record.id)
